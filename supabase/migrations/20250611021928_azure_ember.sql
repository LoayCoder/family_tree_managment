/*
  ...
  1. Changes
     - Updates user_profiles table with approval-related columns if they don't exist
     - Creates or replaces functions for user management
     - Properly handles policy creation with existence checks
     - Creates a safe view for user profiles

  2. Security
     - Ensures proper row level security
     - Implements privilege escalation prevention
     - Sets up admin and user-specific policies
*/
...
BEGIN
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can read own profile" ON user_profiles FOR SELECT TO authenticated USING (auth.uid() = id)';
  END IF;
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can insert own profile" ON user_profiles FOR INSERT TO authenticated WITH CHECK (auth.uid() = id)';
  END IF;
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Users can update own profile" ON user_profiles FOR UPDATE TO authenticated USING (auth.uid() = id)';
  END IF;

  -- Check and create "Admins can read all profiles" policy
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Admins can read all profiles" ON user_profiles FOR SELECT TO authenticated USING ((auth.uid() = id) OR is_admin(auth.uid()))';
  END IF;

  -- Check and create "Admins can update all profiles" policy
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
  ...
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Admins can update all profiles" ON user_profiles FOR UPDATE TO authenticated USING (is_admin(auth.uid()))';
  END IF;

  -- Check and create "Admins can delete profiles" policy
  SELECT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_profiles' AND policyname = 'Admins can delete profiles'
  ) INTO policy_exists;
  
  IF NOT policy_exists THEN
    EXECUTE 'CREATE POLICY "Admins can delete profiles" ON user_profiles FOR DELETE TO authenticated USING (is_admin(auth.uid()))';
  END IF;
END $$;
...
-- Create a safe view for user profiles (drop first if exists)
